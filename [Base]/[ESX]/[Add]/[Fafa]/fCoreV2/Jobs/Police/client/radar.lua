...
function radarSetSpeed(defaultText)
    Visual.KeyboardNumber(TranslationJob.Police["MaxSpeedRadar"], "", 10)
    ...
    if (GetOnscreenKeyboardResult()) then
        ...
        if gettxt ~= nil then
            ...
            AddTextComponentString(TranslationJob.Police["EnterCorectNumber"])
...
function radarPolice()
    if isRadarPlaced then 
        if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), RadarPos.x, RadarPos.y, RadarPos.z, true) < 0.9 then 
            ...
            ClearPrints()
            SetTextEntry_2("STRING")
            AddTextComponentString(TranslationJob.Police["notNextToRadar"])
            DrawSubtitleTimed(3000, 1)
            Citizen.Wait(1500) 
        end
    else 
    ...
    while true do
        ...
        if isRadarPlaced then
            ...
            if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), RadarPos.x, RadarPos.y, RadarPos.z, true) > 300 then 
                ...
                ClearPrints()
                SetTextEntry_2("STRING")
                AddTextComponentString(TranslationJob.Police["GoneTooFarFromRadar!"])
                DrawSubtitleTimed(3000, 1)
            end
        end
        if isRadarPlaced then
            ...
            if veh ~= nil then
                ...
                if vehSpeedKm < maxSpeed then
                    info = string.format(TranslationJob.Police["FirstVehiclePlate"], vehDesc, vehPlate, math.ceil(vehSpeedKm))
                else
                    info = string.format(TranslationJob.Police["SecondVehiclePlate"], vehDesc, vehPlate, math.ceil(vehSpeedKm))
                    ...
                    LastInfo = string.format(TranslationJob.Police["SecondVehiclePlate"], LastVehDesc, LastPlate, math.ceil(LastSpeed))
                end
