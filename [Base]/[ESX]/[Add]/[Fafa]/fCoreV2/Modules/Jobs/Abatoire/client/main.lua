CreateThread(function()
    while true do 
        ...
        for k,v in pairs(MarkerAbatoire.Patron) do
            ...
            if dist <= v.distMax3D then 
                if ESX.PlayerData.job and ESX.PlayerData.job.name == v.jobName then 
                    ...
                    if IsControlJustPressed(1, 51) then 
                        openMenuGarageAbatoire()
                    end
                end
            end
        end
        for k,v in pairs(MarkerAbatoire.Delete) do
            local dist = #(v.Point-PlayerPos) 
            if dist <= v.maxDistMarker then 
                if IsPedInVehicle(PlayerPedId(), vehicleAbatoire, true) then
                    interval = 0
                    ...
                    if dist <= v.distMax3D then 
                        ...
                        if IsControlJustPressed(1, 51) then 
                            ...
                            Wait(1000)
                            ESX.Game.DeleteVehicle(vehicleAbatoire)
                            DoScreenFadeIn(1000)
                            TriggerServerEvent("fCore:Abatoire:Caution", v.caution, 2)
                        end
                    end
                end
            end
