...
passport.addEventListener('click', () => {
  ...
  // Vérifie si la page secondaire du passeport est ouverte
  const secondaryPage = passport.querySelector('.passport__front-cover-face.passport__front-cover-backface');
  const secondaryPageVisible = window.getComputedStyle(secondaryPage).getPropertyValue('display') !== 'none';
  
  if (secondaryPageVisible) {
    document.body.classList.add('cursor-visible');
    document.body.style.cursor = 'auto'; // Assure que le curseur est visible
  } else {
    document.body.classList.remove('cursor-visible');
    document.body.style.cursor = 'none'; // Cache le curseur si nécessaire
  }
});
