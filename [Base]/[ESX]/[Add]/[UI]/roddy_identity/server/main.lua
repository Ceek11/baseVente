...
RegisterNetEvent("fCore:idUnique:giveId")
AddEventHandler("fCore:idUnique:giveId", function()
    local _src = source 
    local xPlayer = ESX.GetPlayerFromId(_src)
    local identifier = xPlayer.identifier
    MySQL.Async.fetchAll("SELECT identifier FROM uniqueid WHERE identifier = @identifier", {["@identifier"] = identifier}, function(result)
		print(json.encode(result))
        if #result == 0 then 
            MySQL.Async.execute("INSERT INTO uniqueid (identifier) VALUES (@identifier)", {
                ["@identifier"] = xPlayer.identifier
            })
        end
    end)
