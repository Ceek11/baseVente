    ...
    while true do
        ...
        if pump then
            ...
            if not holdingNozzle and not nozzleInVehicle and not nozzleDropped then
                ...
                if not weapon then
                    jerryCanText = " \nAppuyez ~b~[G] ~s~pour ~g~acheter ~s~un bidon \nà ~g~" .. price .. "$"
                elseif weapon and GetSelectedPedWeapon(ped) == 883325847 and ammo < 4500 then
                    ...
                    jerryCanText = " \nAppuyez ~b~[G] ~s~pour ~g~remplir ~s~le bidon \nà ~g~" .. price .. "$"
                end
                DrawText3D(pump.x, pump.y, pump.z + 1.2, "~s~Appuyez ~b~[E] ~s~pour ~g~mettre ~s~de l'essence" .. jerryCanText)
            ...
            elseif holdingNozzle and not nearTank and pumpHandle == usedPump then
                DrawText3D(pump.x, pump.y, pump.z + 1.2, "~s~Appuyez ~b~[E] ~s~pour ~r~déposer ~s~la pompe ~s~à essence")
                if IsControlJustPressed(0, 51) then
                    LoadAnimDict("anim@am_hold_up@male")
                    TaskPlayAnim(ped, "anim@am_hold_up@male", "shoplift_high", 2.0, 8.0, -1, 50, 0, 0, 0, 0)
                    Wait(300)
    ...
    while true do
        ...
        if holdingNozzle or nozzleInVehicle or nozzleDropped then
            ...
            if nozzle and pumpCoords then
                ...
                if #(pumpCoords - pedCoords) < 6.0 then
                    SendNUIMessage({
                        type = "status",
                        status = true
                    })
                ...
                if nozzleDropped and #(nozzleLocation - pedCoords) < 1.5 then
                    DrawText3D(nozzleLocation.x, nozzleLocation.y, nozzleLocation.z, "Appuyez ~b~[E] ~s~pour ~g~ramasser ~s~la ~o~pompe")
                    if IsControlJustPressed(0, 51) then
            ...
            if veh then
                ...
                if tankPosition and #(pedCoords - tankPosition) < 1.2 then
                    if not nozzleInVehicle and holdingNozzle then
                        nearTank = true
                        DrawText3D(tankPosition.x + textModifiedPosition.x, tankPosition.y + textModifiedPosition.y, tankPosition.z + zPos + textModifiedPosition.z, "~s~Appuyez ~b~[E] ~s~pour ~g~remplir ~s~le reservoir")
                        if IsControlJustPressed(0, 51) then
                            LoadAnimDict("timetable@gardener@filling_can")
                            TaskPlayAnim(ped, "timetable@gardener@filling_can", "gar_ig_5_filling_can", 2.0, 8.0, -1, 50, 0, 0, 0, 0)
                            Wait(300)
                    ...
                    elseif nozzleInVehicle then
                        DrawText3D(tankPosition.x + textModifiedPosition.x, tankPosition.y + textModifiedPosition.y, tankPosition.z + zPos + textModifiedPosition.z, "~s~Appuyez ~b~[E] ~s~pour ~r~récupérer ~s~la pompe ~s~à essence")
                        if IsControlJustPressed(0, 51) then
                            LoadAnimDict("timetable@gardener@filling_can")
                            TaskPlayAnim(ped, "timetable@gardener@filling_can", "gar_ig_5_filling_can", 2.0, 8.0, -1, 50, 0, 0, 0, 0)
                            Wait(300)
        ...
        if GetSelectedPedWeapon(ped) == 883325847 and not holdingNozzle and not nozzleInVehicle then
            ...
            if veh then
                ...
                if tankPosition and #(pedCoords - tankPosition) < distance then
                    ...
                    DrawText3D(tankPosition.x, tankPosition.y, tankPosition.z + zPos, math.floor(fuel) .. "%, Appuyez ~b~[E] ~s~pour ~g~remplir ~s~le reservoir")
                    local ammo = GetAmmoInPedWeapon(ped, 883325847)
